...
export default function PublicPreview() {
  ...
  async function handleImageChange(position, file) {
    ...
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) { setUploadError('Only JPG, PNG and WebP images are allowed'); return }
    setUploadError('')
    setUploadSuccess('')
    ...
    try {
      ...
      await updateItem(featured.item.id, fd)
      setUploadSuccess(`Image updated for slot ${position}`)
      await fetchFeatured()
    } catch { setUploadError('Failed to upload image. Please try again.') }
  ...
  async function handleSetFeaturedItem(position, itemId) {
    ...
    try {
      ...
      setShowPicker(null)
      setPickerSearch('')
    } catch { setUploadError('Failed to update slot.') }
  ...
  async function handleRemoveSlot(position) {
    ...
    try {
      ...
      await fetchFeatured()
    } catch { setUploadError('Failed to remove slot.') }
  }

  const pickerFiltered = allItems.filter(i =>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-gold/20 bg-gradient-to-r from-brown to-brown-light px-6 py-4 flex items-center justify-between">
        <div>
          <p className="font-playfair font-bold text-gold text-base">Featured Menu — 4 Slots</p>
          <p className="text-cream/50 text-sm mt-1">These 4 items appear on your public website. Change the item or its image at any time.</p>
        </div>
      ...
      {uploadError && (
        <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-sm flex items-center justify-between">
          ...
          <button onClick={() => setUploadError('')} className="text-red-400 hover:text-red-600 font-bold text-lg">&times;</button>
        </div>
      )}
      ...
      {uploadSuccess && (
        <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-700 text-sm flex items-center justify-between">
          ...
          <button onClick={() => setUploadSuccess('')} className="text-emerald-400 hover:text-emerald-600 font-bold text-lg">&times;</button>
        </div>
      )}
        ...
        {[1, 2, 3, 4].map(position => {
          ...
          return (
            <div key={position} className="ad-card overflow-hidden">
                ...
                <div className="relative h-48 overflow-hidden bg-cream-dark group">
                  {featured.item.image_url
                    ...
                    : <div className="w-full h-full flex flex-col items-center justify-center gap-2">
                        ...
                        <p className="text-xs text-brown/30 font-inter">No image yet</p>
                      </div>
                  }
                  <div className="absolute inset-0 bg-brown/60 opacity-0 group-hover:opacity-100 transition-all duration-200 flex flex-col items-center justify-center gap-3">
                    {uploadingId === position
                      ? <><Spinner size="sm" /><p className="text-cream text-xs font-inter">Uploading…</p></>
                      : <>
                          <button
              ...
              {featured ? (
                <div className="p-4">
                  <p className="text-xs text-brown/40 font-inter uppercase tracking-wide mb-1">{featured.item.category_name}</p>
                  <p className="font-playfair font-bold text-brown text-sm leading-tight mb-1 truncate">{featured.item.name}</p>
                  <p className="font-inter font-bold text-gold text-sm">Rs. {parseFloat(featured.item.price).toFixed(2)}</p>
                  <div className="flex items-center justify-between mt-3 pt-3 border-t border-cream-dark">
                    <span className={featured.item.is_available
      ...
      {showPicker !== null && (
        <div
          style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
          onClick={() => { setShowPicker(null); setPickerSearch('') }}
        >
          <div
            style={{ background: 'white', borderRadius: '16px', padding: '24px', width: '100%', maxWidth: '480px', maxHeight: '70vh', overflowY: 'auto' }}
              ...
              {pickerFiltered.map(item => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="font-inter font-bold text-gold text-sm flex-shrink-0">Rs. {parseFloat(item.price).toFixed(2)}</span>
                  </button>
                )
              })}
            </div>
      ...
      <div className="rounded-2xl border border-gold/20 bg-cream p-5 flex items-start gap-4">
        <div className="w-9 h-9 rounded-xl bg-gold/10 flex items-center justify-center text-lg flex-shrink-0">ℹ️</div>
        <div>
